...
public class ImageLoader {
    ...
    private Bitmap getBitmap(String url) {
        ...
        if (result == null) {
            ...
            if (result == null) {
            	String netWorkState = ConnectionDetector.getNetWorkState(mContext);
            	SharedPreferences settings = mContext.getSharedPreferences(MySharedPreferences.SETTINGS, 0);
            	boolean areGetBitmap = settings.getBoolean("areGetBitmap", true);
            	Log.e("areGetBitmap", areGetBitmap + "");
            	if(areGetBitmap || "WIFI连接".equals(netWorkState)){
            		 // 从网络获取
                    result = ImageGetFromHttp.downloadBitmap(url);
                    if (result != null) {
                    	Log.e("getPic:" + url + "from", "Http");
